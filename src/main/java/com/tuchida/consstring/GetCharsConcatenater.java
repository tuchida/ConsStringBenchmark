...
public class GetCharsConcatenater implements Concatenater {

    static class FixedStringBuilder {
        private char[] buf;
        private int pos = 0;

        public FixedStringBuilder(int size) {
            buf = new char[size];
        }
 ...
 */
static class ConsString implements CharSequence, Serializable {
    ...
    private synchronized String flatten() {
        if (depth > 0) {
            FixedStringBuilder buf = new FixedStringBuilder(length);
            appendTo(buf);
            s1 = buf.toString();
            s2 = "";
            depth = 0;
    ...
    private synchronized void appendTo(FixedStringBuilder buf) {
        appendFragment(s1, buf);
        appendFragment(s2, buf);
    }

    private static void appendFragment(CharSequence s, FixedStringBuilder buf) {
        if (s instanceof ConsString) {
            ((ConsString)s).appendTo(buf);
        } else {
    ...
    @Override
    public CharSequence concat(CharSequence s1, CharSequence s2) {
        return new ConsString(s1, s2);
    }
}
